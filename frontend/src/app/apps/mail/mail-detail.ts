...
@Component({
    standalone: true,
    imports: [AvatarModule, ButtonModule, RippleModule, EditorModule, FormsModule],
    template: `@if (mail) {
        <div>
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 pt-8 md:pt-0 gap-6 md:border-t-0 border-t border-surface-200 dark:border-surface-700">
                <div class="flex items-center md:justify-start">
                    ...
                    @if (mail && mail.image) {
                        <p-avatar [image]="'/demo/images/avatar/' + mail.image" size="large" shape="circle" styleClass="border-2 border-surface-200 dark:border-surface-700"></p-avatar>
                    }
                ...
                <div class="flex items-center justify-end gap-x-4 px-6 md:px-0">
                    ...
                    <button pButton pRipple type="button" icon="pi pi-reply" class="shrink-0" text plain></button>
                    <button pButton pRipple type="button" icon="pi pi-ellipsis-v" class="shrink-0" text plain></button>
                </div>
            </div>
